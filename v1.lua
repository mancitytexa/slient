...
local Watermark = Library:Watermark({Name = "| Sharp.cc | Sharp"})
--
Window:Seperator({Name = "Aimbot"})
local Aim = Window:Page({Name = "General"})
Window:Page({Name = "Misc", Icon = "http://www.roblox.com/asset/?id=6023426921"})
Window:Page({Name = "Other", Icon = "http://www.roblox.com/asset/?id=6031280882"})
Window:Seperator({Name = "Visuals"})
local Players = Window:Page({Name = "Box", Icon = "http://www.roblox.com/asset/?id=6023426944"})
Window:Page({Name = "View", Icon = "http://www.roblox.com/asset/?id=6031075931"})
Window:Seperator({Name = "Miscellaneous"})
local Other = Window:Page({Name = "Other", Icon = "http://www.roblox.com/asset/?id=6031280882"})
--
local Sec = Aim:Section({Name = "General", Size = 380})
local Sec2 = Aim:Section({Name = "Other", Size = 186, Side = "Right"})
local Sec3 = Aim:Section({Name = "Misc", Size = 186, Side = "Right"})
--
local Toggle = Sec:Toggle({Name = "Enable", Callback = function(State) print(State) end})
local OptList = Toggle:OptionList({})
OptList:Toggle({Name = "Checkbox"})
Sec:Toggle({Name = "Checkbox"})
Sec:Slider({Name = "Slider Float", Min = 0, Max = 100, Callback = function(State) print(State) end})
Sec:Slider({Name = "Slider Integer", Min = 0, Max = 100, Decimals = 0.01, Callback = function(State) print(State) end})
Sec:List({Name = "Combo", Callback = function(State) print(State) end})
Sec:List({Name = "Multi-Combo", Max = 3, Callback = function(State) print(State) end})
Sec:Colorpicker({Name = "Color", Callback = function(State) print(State) end})
Sec:Keybind({Name = 'Hold-Keybind', Default = Enum.KeyCode.E, Mode = "Hold", Callback = function(State) print(State) end})
Sec:Keybind({Name = 'Toggle-Keybind', Default = Enum.KeyCode.R, Mode = "Toggle", Callback = function(State) print(State) end})
OptList:Slider({Name = "Slider", Callback = function(State) print(State) end})
OptList:List({Name = "Combo", Callback = function(State) print(State) end})
--
Sec3:Textbox({Placeholder = "Text Field", Callback = function(State) print(State) end})
Sec3:Button({Name = "New Notification", Callback = function() Library:Notification("Notification!!!", 5) end})
--
local Nest2 = OptList:Nest({Size = 150})
local Nest = Sec2:Nest({Size = 150})
--
Nest2:Toggle({Name = "a"})
Nest2:Toggle({Name = "adasd"})
Nest2:Toggle({Name = "nn"})
Nest2:Toggle({Name = "scroll"})
Nest2:Toggle({Name = "So yop"})
Nest2:Slider({Name = "slide"})
--
for i = 1,20 do
	Nest:Toggle({Name = tostring(i)})
end
--
Players:Section({Name = "Global", Size = 380, Side = "Right"})
local Enemies = Players:Section({Name = "Enemies", Size = 186})
Players:Section({Name = "Friendlies", Size = 186})

-- SETTINGS PAGE
do
	local Config = Other:Section({Name = "Config", Size = 380})
    --[[
        Config Functions:
        Library:GetConfig() -- Grabs all settings  Ex. writefile("config.txt",Library:GetConfig())
        Library:LoadConfig(Config) -- Loads config file  Ex. Library:LoadConfig(readfile("config.txt"))
    --]]
	--
	Config:Keybind({Name = "Menu Key", Flag = "MenuKey", Default = Enum.KeyCode.End, UseKey = true, Callback = function(State) Library.UIKey = State end})
	Config:Colorpicker({Name = "Menu Accent", Flag = "MenuAccent", Default = Color3.fromRGB(10, 162, 252), Callback = function(State) Library:ChangeAccent(State) end})
	Config:Toggle({Name = "Watermark", Flag = "Watermark", Callback = function(State) Watermark:SetVisible(State) end})
	Config:Textbox({Placeholder = "Watermark Text", Default = "| beta | username", Callback = function(State) Watermark:UpdateText(State) end})
end
